...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                safeClamTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
    ...
    // Withdraw Clam tokens from STAKING.
    ...
    function safeClamTransfer(address _to, uint256 _amount) internal {
        uint256 clamBal = clam.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > clamBal) {
            transferSuccess = clam.transfer(_to, clamBal);
        } else {
            transferSuccess = clam.transfer(_to, _amount);
        }
        require(transferSuccess, "safeClamTransfer: transfer failed");
    }
